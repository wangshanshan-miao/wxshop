...
Page({
  ...
  onLoad: function (options) {
    const goodType = options.classId
    console.log(options)
    this.setData({
      baseURL,
      imgBaseUrl,
  ...
  getList(item) {
    api.getAllClass({
      ...
      // this.getShopList(this.data.goodType)
      this.toTab(this.data.activeIndex)
    })
  },
  getShopList(classId) {
    api.searchResult({
  ...
  toTab (tabIndex) {
    ...
    this.triggerEvent("switchTap", this.data.goodType); //点击了导航,通知父组件重新渲染列表数据
    this.getShopList(this.data.goodType)
  },
  switchTap(e) { //更换资讯大类
    this.setData({
